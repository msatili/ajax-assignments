...
dc.loadAbout = function () {
  ...
  $ajaxUtils.sendGetRequest(
    ...
    function (aboutHtmlUrl) {
      ...
      switchAboutToActive();
      //var aboutViewHtml =
      var htmlToInsert = buildAboutViewHtml(aboutHtmlUrl);
...
function buildAboutViewHtml (html) {
  // Generate a random number from 1-5
  var randomNumber = Math.floor((Math.random() * 5) + 1);
  var filledStar = '"fa fa-star"';
  var emptyStar = '"fa fa-star-o"';

  // Replace empty class values in spans with {{}}
  // so that insertProperty function can be used
  // to replace with filled stars based on rating
  for(var i = 0; i < randomNumber; i++) {
    html = html.replace('""', "{{}}");
  }

  // Change all instances of {{}} in about snippet
  // to filled star
  html = insertProperty(html, "", filledStar);

  // Replace remaining empty class values in spans with {{}}
  // so that insertProperty function can be used
  // to replace with empty stars based on rating
  for(i = randomNumber; i < 5; i++) {
    html = html
      .replace('""', "{{}}");
  }
  ...
  // Change all instances of {{}} in about snippet
  // to empty star
  html = insertProperty(html, "", emptyStar);

  // Add a numeric string to represent the rating
  html += " " + randomNumber + "-star rating</center>";
